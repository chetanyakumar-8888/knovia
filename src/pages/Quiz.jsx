...
const Quiz = () => {
  ...
  const [selectedClass, setSelectedClass] = useState(location.state?.selectedClass || '10');
  const [selectedSubject, setSelectedSubject] = useState(location.state?.selectedSubject || '');
  const [selectedChapter, setSelectedChapter] = useState(location.state?.selectedChapter || '');
    ...
    try {
      ...
      if (mcqs && mcqs.length > 0) {
        const sanitizedMcqs = mcqs.map(q => {
          ...
          return { ...q, correctAnswer: (typeof correctIdx === 'number' && correctIdx >= 0 && correctIdx < q.options.length) ? correctIdx : 0 };
        });
  ...
  const handleSelectAnswer = (index) => {
    ...
    setIsAnswerRevealed(true);
    if (index === quizQuestions[currentQuestion].correctAnswer) setScore(s => s + 1);
  };
  ...
  const handleNextQuestion = async () => {
    if (currentQuestion < quizQuestions.length - 1) {
      ...
      setStep('results');
      try {
        await saveQuizResult(selectedSubject, selectedChapter, selectedClass, score, quizQuestions.length);
      } catch (err) {
        ...
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          {step === 'quiz' && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900 border border-white/10">
              ...
              <span className={`text-sm font-medium ${timeLeft <= 10 ? 'text-red-400' : 'text-slate-200'}`}>{timeLeft}s</span>
            </div>
          )}
        </div>
      </nav>
        ...
        {step === 'start' && (
          <div className="animate-[fade-in_0.3s_ease-out] space-y-6">
            ...
            <div className="bg-slate-900/50 border border-white/5 rounded-3xl p-6 sm:p-8">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select value={selectedClass}
                      ...
                      className="appearance-none w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white pr-10 focus:outline-none focus:border-purple-500 cursor-pointer">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select value={selectedSubject}
                      ...
                      className="appearance-none w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white pr-10 focus:outline-none focus:border-purple-500 cursor-pointer disabled:opacity-50">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select value={selectedChapter}
                      ...
                      className="appearance-none w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white pr-10 focus:outline-none focus:border-purple-500 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              {/* Difficulty */}
              <div className="mb-6">
                <label className="block text-sm font-medium text-slate-400 mb-3">Difficulty</label>
                <div className="grid grid-cols-3 gap-3">
                  {['Easy', 'Medium', 'Hard'].map(diff => (
                    <button key={diff} onClick={() => setDifficulty(diff)}
                      className={`py-3 rounded-xl border font-medium text-sm transition-all ${difficulty === diff ? 'bg-purple-500/20 border-purple-500 text-purple-300' : 'bg-slate-950 border-white/10 text-slate-400 hover:border-white/20'}`}>
                      {diff}
                    </button>
                  ))}
                </div>
              ...
              <button onClick={handleStartQuiz} disabled={isLoading || !selectedChapter}
                className="w-full py-4 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-lg transition-all shadow-[0_0_20px_rgba(147,51,234,0.3)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                {isLoading ? (
                  <>
            ...
            <div className="space-y-4 mb-8">
              {quizQuestions[currentQuestion].options.map((option, idx) => {
                ...
                if (!isAnswerRevealed) {
                  ...
                  if (isCorrect) optionClasses += "bg-emerald-500/10 border-emerald-500 text-emerald-300";
                  else if (isSelected && !isCorrect) optionClasses += "bg-rose-500/10 border-rose-500 text-rose-300";
                  else optionClasses += "bg-slate-900/20 border-white/5 opacity-50";
                }
                return (
                  <button key={idx} onClick={() => handleSelectAnswer(idx)} disabled={isAnswerRevealed} className={optionClasses}>
                    <div className="flex items-center gap-4">
        ...
        {step === 'results' && (
          <div className="bg-slate-900/50 border border-white/5 rounded-3xl p-8 sm:p-12 text-center animate-[fade-in_0.5s_ease-out] relative overflow-hidden">
            ...
            <p className="text-slate-500 mb-10">
              {score / quizQuestions.length >= 0.7 ? "Outstanding! You're ready for the exam. 🌟" : "Good effort! Review the notes and try again. 💪"}
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <button onClick={handleStartQuiz}
                ...
                <RotateCcwIcon className="w-5 h-5 text-slate-400" /> Try Again
              </button>
              <button onClick={() => navigate('/study')}
                className="px-6 py-3.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold transition-colors flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(147,51,234,0.3)]">
