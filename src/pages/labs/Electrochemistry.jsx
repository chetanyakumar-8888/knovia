    ...
    if (running) {
      ...
      for (let i = 0; i < numDots; i++) {
        ...
        if (pathPos < W - 260) {
          ex = 130 + pathPos;
          ey = 40;
        } else if (pathPos < W - 260 + 50) {
          ex = W - 130;
          ey = 40 + (pathPos - (W - 260));
        } else {
          ex = W - 130 - (pathPos - (W - 260 + 50));
          ey = 90;
        }
    ...
    <div className="min-h-screen bg-gray-950 text-white p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          >←</button>
          <h1 className="text-xl font-bold">Electrochemistry ⚡🧪</h1>
        </div>
      ...
      <div className="flex gap-4 mb-4">
                ...
                {CELLS.map((c, i) => (
                  <button key={i} onClick={() => { setCellType(i); setTick(0); }}
                    className={`w-full text-left px-3 py-2 rounded-lg text-xs font-medium ${
                      cellType === i ? "bg-purple-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
                    }`}>{c.name} ({c.emf}V)</button>
                ))}
              </div>
            </div>
        ...
        <div className="w-2/3 bg-gray-900 rounded-xl border border-gray-800 p-4">
          <h2 className="text-sm font-semibold text-gray-400 mb-3">
            {cell.name} — Electrochemical Cell
          </h2>
          <canvas ref={canvasRef} width={600} height={320}
            className="w-full rounded-lg bg-gray-950"/>
          <div className="flex gap-4 mt-2 text-xs">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 bg-yellow-400 rounded-full inline-block"></span> Electrons
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 bg-green-400 rounded-full inline-block"></span> Cations (+)
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 bg-red-400 rounded-full inline-block"></span> Anions (−)
            </span>
          </div>
        </div>
      </div>
