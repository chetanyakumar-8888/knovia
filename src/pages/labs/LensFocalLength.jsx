    ...
    <div className="min-h-screen bg-gray-950 text-white pb-20">
      ...
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-gray-950/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-base sm:text-xl font-bold">Lens & Focal Length 🔬</h1>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6">
        ...
        <div className="w-full bg-gray-900 rounded-xl border border-gray-800 p-4 mb-6 lg:hidden">
          ...
          <canvas ref={canvasRef} width={700} height={320} className="w-full rounded-lg bg-gray-950" />
            ...
            <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
                ...
                <div className="flex gap-2">
                  {["convex", "concave"].map((type) => (
                    <button key={type} onClick={() => setLensType(type)}
                      className={`flex-1 py-2 rounded-lg text-sm font-medium capitalize ${lensType === type ? "bg-purple-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>
                      {type}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-4">
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-yellow-400 bg-yellow-900/40 px-2 py-0.5 rounded">{objectDistance} cm</span>
                </div>
                <input type="range" min="5" max="100" value={objectDistance}
                  onChange={(e) => setObjectDistance(Number(e.target.value))}
                  className="w-full accent-yellow-500" />
                <div className="flex justify-between text-xs text-gray-600 mt-1"><span>5 cm</span><span>100 cm</span></div>
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm text-gray-400">Focal Length (f)</span>
                  <span className="text-sm font-bold text-green-400 bg-green-900/40 px-2 py-0.5 rounded">{focalLength} cm</span>
                </div>
                <input type="range" min="5" max="50" value={focalLength}
                  onChange={(e) => setFocalLength(Number(e.target.value))}
                  className="w-full accent-green-500" />
                <div className="flex justify-between text-xs text-gray-600 mt-1"><span>5 cm</span><span>50 cm</span></div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            <p className="text-sm text-gray-300 mb-2">
              <strong className="text-white">Lens Formula:</strong> 1/v - 1/u = 1/f relates object distance, image distance and focal length.
            </p>
            <div className="bg-gray-950 rounded-lg p-3 font-mono text-center text-sm mb-2">
              <span className="text-purple-400">m = v/u = h'/h</span>
            </div>
            <p className="text-sm text-gray-400">Convex lens converges light rays. Concave lens diverges light rays.</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
