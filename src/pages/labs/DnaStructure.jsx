  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      SEQUENCE.forEach((base, i) => {
        ...
        // Hydrogen bonds (dashed line between bases)
        ctx.beginPath();
        ctx.moveTo(lx, y);
        ctx.lineTo(rx, y);
        ...
        // Connect to next base on same strand
    ...
    <div className="min-h-screen bg-gray-950 text-white p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          >←</button>
          <h1 className="text-xl font-bold">DNA Structure 🧬</h1>
        </div>
        ...
        <div className="w-1/3 flex flex-col gap-4">
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            <button
              ...
              className={`w-full py-2 rounded-lg text-sm font-medium mb-4 ${
              ...
              {[["A","T"], ["G","C"]].map(([b1, b2]) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-black"
                      style={{ backgroundColor: BASES[b2].color }}
                    >{b2}</span>
                  </div>
                </div>
              ))}
            ...
            <div className="flex flex-wrap gap-1">
              {SEQUENCE.map((base, i) => (
                <span
                  ...
                  style={{ backgroundColor: BASES[base].color }}
                >{base}</span>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {SEQUENCE.map((base, i) => (
                <span
                  key={i}
                  className="w-6 h-6 rounded flex items-center justify-center text-xs font-bold text-black"
                  style={{ backgroundColor: BASES[BASES[base].pair].color }}
                >{BASES[base].pair}</span>
              ))}
            </div>
          </div>
        </div>
