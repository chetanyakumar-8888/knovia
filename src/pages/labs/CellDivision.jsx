...
const PHASES = [
  { name: "Interphase", color: "#7c3aed", description: "Cell prepares for division. DNA replication occurs. Cell grows in size.", chromosomes: "duplicated" },
  { name: "Prophase", color: "#2563eb", description: "Chromosomes condense and become visible. Nuclear envelope breaks down.", chromosomes: "condensing" },
  { name: "Metaphase", color: "#0891b2", description: "Chromosomes align at the cell's equator (metaphase plate).", chromosomes: "aligned" },
  { name: "Anaphase", color: "#059669", description: "Sister chromatids separate and move to opposite poles of the cell.", chromosomes: "separating" },
  { name: "Telophase", color: "#d97706", description: "Nuclear envelopes reform around each set of chromosomes.", chromosomes: "reforming" },
  { name: "Cytokinesis", color: "#dc2626", description: "Cytoplasm divides forming two identical daughter cells.", chromosomes: "divided" },
];
...
const CellDivision = () => {
  ...
  useEffect(() => {
    ...
    if (phase === 0) {
      ...
      for (let i = 0; i < 4; i++) {
        ctx.beginPath(); ctx.ellipse(cx + (i-1.5)*12, cy, 4, 15, i*0.4, 0, Math.PI*2);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const x = cx + Math.cos(angle) * 30; const y = cy + Math.sin(angle) * 30;
        ctx.beginPath(); ctx.ellipse(x, y, 8, 18, angle, 0, Math.PI * 2);
        ctx.fillStyle = "#93c5fd"; ctx.fill();
        ctx.strokeStyle = "#1d4ed8"; ctx.lineWidth = 1; ctx.stroke();
      }
    } else if (phase === 2) {
      ctx.beginPath(); ctx.arc(cx, cy, 100, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(8, 145, 178, 0.15)"; ctx.fill();
      ctx.strokeStyle = "#0891b2"; ctx.lineWidth = 3; ctx.stroke();
      ctx.beginPath(); ctx.moveTo(cx, cy - 100); ctx.lineTo(cx, cy + 100);
      ctx.strokeStyle = "rgba(8, 145, 178, 0.4)"; ctx.lineWidth = 1;
      ctx.setLineDash([4, 4]); ctx.stroke(); ctx.setLineDash([]);
      for (let i = 0; i < 4; i++) {
        const y = cy - 45 + i * 30;
        ctx.beginPath(); ctx.ellipse(cx - 12, y, 7, 12, 0, 0, Math.PI * 2); ctx.fillStyle = "#67e8f9"; ctx.fill();
        ctx.beginPath(); ctx.ellipse(cx + 12, y, 7, 12, 0, 0, Math.PI * 2); ctx.fillStyle = "#67e8f9"; ctx.fill();
        ctx.beginPath(); ctx.arc(cx, y, 4, 0, Math.PI * 2); ctx.fillStyle = "#0e7490"; ctx.fill();
      }
      for (let i = 0; i < 4; i++) {
        const y = cy - 45 + i * 30;
        ctx.beginPath(); ctx.moveTo(cx, cy - 90); ctx.lineTo(cx, y);
        ctx.strokeStyle = "rgba(103, 232, 249, 0.4)"; ctx.lineWidth = 1; ctx.stroke();
        ctx.beginPath(); ctx.moveTo(cx, cy + 90); ctx.lineTo(cx, y); ctx.stroke();
      }
    } else if (phase === 3) {
      ctx.beginPath(); ctx.ellipse(cx, cy, 130, 90, 0, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(5, 150, 105, 0.15)"; ctx.fill();
      ctx.strokeStyle = "#059669"; ctx.lineWidth = 3; ctx.stroke();
      for (let i = 0; i < 4; i++) {
        const x = cx - 30 + i * 20;
        ctx.beginPath(); ctx.ellipse(x, cy - 45, 7, 14, 0, 0, Math.PI * 2); ctx.fillStyle = "#6ee7b7"; ctx.fill();
        ctx.beginPath(); ctx.ellipse(x, cy + 45, 7, 14, 0, 0, Math.PI * 2); ctx.fillStyle = "#6ee7b7"; ctx.fill();
      }
    } else if (phase === 4) {
      ctx.beginPath(); ctx.ellipse(cx, cy, 130, 90, 0, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(217, 119, 6, 0.15)"; ctx.fill();
      ctx.strokeStyle = "#d97706"; ctx.lineWidth = 3; ctx.stroke();
      ctx.beginPath(); ctx.arc(cx, cy - 40, 35, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(217, 119, 6, 0.2)"; ctx.fill();
      ...
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-gray-950/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-base sm:text-xl font-bold">Cell Division (Mitosis) 🔬</h1>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6">
          ...
          <div className="w-full lg:w-1/3 flex flex-col gap-4">
            <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
              ...
              <div className="space-y-2 mb-4">
                {PHASES.map((p, i) => (
                  <button key={p.name} onClick={() => { setPhase(i); setAuto(false); }}
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-all ${phase === i ? "text-white border-2" : "bg-gray-800 text-gray-400 hover:bg-gray-700 border border-gray-700"}`}
                    style={phase === i ? { backgroundColor: p.color + "33", borderColor: p.color, color: p.color } : {}}>
                    {i + 1}. {p.name}
                  </button>
                ))}
              </div>
              <button onClick={() => setAuto(!auto)}
                className={`w-full py-2 rounded-lg text-sm font-medium ${auto ? "bg-red-600 hover:bg-red-700" : "bg-purple-600 hover:bg-purple-700"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            <p className="text-sm text-gray-400">It occurs in somatic (body) cells for growth, repair and asexual reproduction.</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
